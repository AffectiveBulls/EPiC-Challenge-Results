...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        # make sure that the training data does not accidentally have any label information
        data_columns = list(set(train_data.columns).difference(set(['valence', 'arousal', 'subject', 'stimulus'])))
        train_x = train_data[data_columns]
        train_valence = train_data[['valence']]
        train_arousal = train_data[['arousal']]
        
        data_columns = list(set(valid_data.columns).difference(set(['valence', 'arousal', 'subject', 'stimulus'])))
        valid_x = valid_data[data_columns]
